...
function scoreFromFinancials(financial, revenueGrowth, fundingStress) {
  ...
  const entornoFinancieroScore = clamp(
    100 - fundingStress * 1.35 - state.issuer.stress.refinancingPressure * 1.2 - state.issuer.stress.marketSensitivity * 0.9,
    5,
    98
  );
  const scoreBase = rentabilidadScore * 0.4 + deudaScore * 0.4 + entornoFinancieroScore * 0.2;
  const psiBruto = calculateRawPsi(financial, revenueGrowth, fundingStress);
...
function calculateRawPsi(financial = latestFinancial(), revenueGrowth = state.incomeGrowth, fundingStress = state.fundingStress) {
  const ratios = calculateFinancialRatios(financial);
  ...
  const rentabilidadScore = clamp(42 + revenueGrowth * 1.25 + ratios.ebitdaMargin * 1.15, 5, 98);
  const delta = getStressDelta(fundingStress);
  const psi =
...
function getStressDelta(fundingStress = state.fundingStress) {
  // Supuestos de stress financiero: no son datos observables directos de mercado.
  // Se usan para simular presion por costo de fondeo, refinanciacion y sensibilidad al entorno.
  return clamp(
    1 -
      (fundingStress * 0.007 +
        state.issuer.stress.refinancingPressure * 0.005 +
        state.issuer.stress.marketSensitivity * 0.003),
    0.55,
    1
  );
}
...
function calculateHistoryScores() {
  return state.issuer.financials.map((financial, index, list) => {
    ...
    return scoreFromFinancials(financial, growth, state.issuer.stress.fundingCost);
  });
}
...
function calculateScenario(incomeGrowth, debtGrowth, fundingStress) {
  ...
  const marginShift = state.analystBias * 0.12 - Math.max(0, fundingStress - state.issuer.stress.fundingCost) * 0.03;
  ...
  for (let year = 1; year <= 3; year += 1) {
    ...
    // El costo financiero proyectado surge de supuestos de stress, no de una tasa observable puntual.
    const interest = debt * clamp((fundingStress + state.issuer.stress.refinancingPressure) / 100, 0.06, 0.55);
    ...
    result.push(scoreFromFinancials(financial, incomeGrowth, fundingStress));
  }
...
function getDiagnosis(values, model) {
  ...
  if (minimum < 55 || model.breachProbability >= 55) {
    return {
      ...
      text: "La combinacion de deuda, cobertura de intereses y stress financiero genera una probabilidad alta de ruptura del flujo seguro.",
    };
  }
...
function getPrescriptions(values, model) {
  ...
  if (gap === 0 && model.breachProbability < 25) {
    return [
      ...
      ["Monitorear costo de fondeo", `Activar alerta si el stress financiero supera ${state.fundingStress + 4} puntos.`],
    ];
  }
...
function renderFinancials() {
  ...
  const ratioRows = [
    ...
    ["Costo financiero", pct((latest.interest / latest.debt) * 100), "Base para supuestos de stress financiero"],
  ];
...
function renderEngineTable(values) {
  ...
  const readings = [
    ...
    [
      "delta Stress Financiero",
      "modificador",
      `Fondeo ${state.fundingStress} · presion refi ${state.issuer.stress.refinancingPressure} · sensibilidad ${state.issuer.stress.marketSensitivity}`,
    ],
  ];
...
function updateStaticStressCopy() {
  const sidebarNote = document.querySelector(".sidebar-note small");
  if (sidebarNote) {
    sidebarNote.textContent = "Psi multiplicativo · Solvencia 40% · Liquidez 33% · Rentabilidad 27% · delta Stress Financiero";
  }
  ...
  document.querySelectorAll("span").forEach((element) => {
    ...
    if (text === "delta Mercado") element.textContent = "delta Stress Financiero";
    if (text === "Riesgo de mercado MAV") element.textContent = "Stress financiero";
    if (text === "Dispersion MAV") element.textContent = "Dispersion stress financiero";
  });

  document.querySelectorAll("p").forEach((element) => {
    element.textContent = element.textContent
      .replaceAll("riesgo MAV", "stress financiero")
      .replaceAll("Riesgo MAV", "Stress financiero")
      .replaceAll("tasa MAV", "costo estimado de fondeo")
      .replaceAll("volatilidad de mercado", "sensibilidad al entorno")
      .replaceAll("mercado", "entorno financiero");
  });
}
