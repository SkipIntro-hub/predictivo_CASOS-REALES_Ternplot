...
function renderPsiPanel() {
  const comp = getPsiComposition();
  const delta = clamp(
    1 - (state.marketRisk * 0.007 + state.issuer.market.issuerSpread * 0.005 + state.issuer.market.volatility * 0.003),
    0.55,
    1
  );
  const psi = Math.round(
    clamp(
      Math.pow(comp.solvencia / 100, weights.solvencia) *
        Math.pow(comp.liquidez / 100, weights.liquidez) *
        Math.pow(comp.rentabilidad / 100, weights.rentabilidad) *
        delta *
        100,
      5,
      98
  ...
  const calibratedPsi = calibratePsiScore(psi);
  ...
  setText("psiScore", `Psi ${psi} · Score ${calibratedPsi}`);
